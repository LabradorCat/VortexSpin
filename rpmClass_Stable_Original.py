...
class ASI_RPM():
    ...
    def graph(self, show = True):
        ...
        #ax_ver.set(adjustable='box', aspect='equal')
        #plt.ticklabel_format(style='sci', scilimits=(0,0))
        #plt.tight_layout()
        #return(graph)
        if show == True:
    ...
    def coerciveVertex(self, Hcmean,colour):
        ...
        ax.scatter(X,Y,c = Vertex[:,:,4].flatten(), marker = 'o', cmap = cm.plasma, zorder=2)
        cb1 = fig.colorbar(graph, fraction=0.046, pad=0.04, ax = ax)
        cb1.locator = MaxNLocator(nbins = 7)
        cb1.update_ticks()
    
        return Hcnew, Hcq
    def Vertex(self,colour):
        #plots vertex type and coercive field on he same plot.
        ...
        graph = ax.quiver(X, Y, Mx, My, angles='xy', scale_units= 'xy',  pivot = 'mid')
        ax.set_xlim([-1*self.unit_cell_len, np.max(X)+self.unit_cell_len])
        ax.set_ylim([-1*self.unit_cell_len, np.max(X)+self.unit_cell_len])
        ax.set_title('Coercive Field')
        ax.scatter(X,Y,c = Vertex[:,:,4].flatten(), marker = 'o', cmap = cm.plasma, zorder=2)
    ...
    def graphChargeAnimate(self, show = False):
    ...
    def singleSpinFlipRPM(self, Hmax, Htheta, loops = 50, n = 6, folder = None, q1 = False):
        ...
        while num_loop < loops:
            ...
            self.singleSpinFlipDynamics(Hmax, Htheta, n =n, folder=folder)
    ...
    def singleSpinFlipRPMchiral(self, Hmax, Htheta, Htheta2, loops = 50, n = 6, folder = None, q1 = False):
        '''
        uses the singleSpinFlipDynamics to do return point memory style minor loops up to Hmax,
        note that Hmax is now the maximum field magnitude and the angle is in radians. 
        The summary data of the end state is saved in a .csv file with hopefully everything that you would ever need.
    ...
    def correlationAll(self, folder, savefolder =None):
        ...
        #saxm.set(adjustable='box', aspect='equal')
    ...
    def fixEdgesModulated(self):
        '''
        Puts the coercive field of the edge spins at 1T
        '''
        x = self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6]
        y = self.lattice[:, (self.side_len_y-3):(self.side_len_y), 6]
        for i in range(len(x)):
            if (i-1)%4 ==0:
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][0][i] == 0.08
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][1][i] == 0.08
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][2][i] == 0.08
                y[0][i] == 0.08
                y[1][i] == 0.08
                y[2][i] == 0.08
            if (i+1)%4 ==0:
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][0][i] == 0.07
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][1][i] == 0.07
                self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6][2][i] == 0.07
                y[0][i] == 0.07
                y[1][i] == 0.07
                y[2][i] == 0.07
        print(self.lattice[(self.side_len_x-3):(self.side_len_x), :, 6])
        #print(x[0][1])
